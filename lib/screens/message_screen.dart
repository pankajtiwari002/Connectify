...
class _MessageScreenState extends State<MessageScreen> {
  ...
  Future<void> getUserData() async {
    ...
    chatUser = await firestoreMethods.getUserDetailsUsingUid(widget.uid);
    setState(() {
      isLoading = false;
    });
  }
                          ...
                          if (snapshot.hasData) {
                            return ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                if (index > 0) {
                                  return MessageCard(
                                    prevdate: snapshot.data!['messages']
                                        [index - 1]['date'],
                                    date: snapshot.data!['messages'][index]
                                        ['date'],
                                    owner: snapshot.data!['messages'][index]
                                        ['owner'],
                                    text: snapshot.data!['messages'][index]
                                        ['text'],
                                    imageUrl: snapshot.data!['messages'][index]
                                        ['imageUrl'],
                                    senderPhotoUrl: user.photoUrl,
                                    userPhotoUrl: chatUser!.photoUrl,
                                  );
                                } else {
                                  return MessageCard(
                                    prevdate: null,
